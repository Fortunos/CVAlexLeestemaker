...
namespace Template
{
	...
	public class Surface
	{
        ...
        public void Circle(float r, float x, float y, int c, float a)
        {
            for (int i = 0; i < a; i++)
            {
                float angle1 = (360 / a) * i;
                float angle2 = (360 / a) * (i + 1);

                int x1 = (int)(x - ((Math.Sin(angle1) * r) / 2));
                int y1 = (int)(y - ((Math.Cos(angle1) * r) / 2));
                int x2 = (int)(x + ((Math.Sin(angle2) * r) / 2));
                int y2 = (int)(y + ((Math.Cos(angle2) * r) / 2));
