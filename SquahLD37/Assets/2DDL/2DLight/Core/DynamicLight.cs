namespace DynamicLight2D {
	...
	public class DynamicLight : MonoBehaviour {
		...
		internal class VertItem {
			...
			List<Vert> _verts = new List<Vert>();
			...
			internal List<Vert> Verts { get { return _verts; } }

			internal int Count { get { return _verts.Count; } }
			...
			internal Vert this[int index] { get { return _verts[index]; } }
		}
		...
		class Collider {
			...
			internal bool DidTransformDeleted { get { return collider == null; } }
			...
			/// <returns></returns>
			internal Vector3 this[int index] { get { return worldPoints[index]; } }
			...
			/// <returns></returns>
			internal VertItem GetVerts(Transform lightObjectTM, float LightRadius, float RangeLightAngle, ref bool sortAngles,
				int _layerMask, float _magRange, ref bool notifyReachedEventUsed, ref List<GameObject> ListObjectsReached,
				ref bool Debugging, bool forceToUpdatePoints) // still to expensive.. improve
			{
				...
				for (int i = 0; i < worldPoints.Length; i++) {
					...
					if (_ray) {
						...
						if (worldLocal.sqrMagnitude >= (rayLocal.sqrMagnitude - rangeRange) &&
							worldLocal.sqrMagnitude <= (rayLocal.sqrMagnitude + rangeRange))
							endpoint = true;
						...
						if (notifyReachedEventUsed && collider.type == CasterCollider.CasterType.PolygonCollider2d ||
							collider.type == CasterCollider.CasterType.BoxCollider2d) {
		...
		/// </summary>
		List<Vert> _allVerts = new List<Vert>();

		/// <summary>
		/// Array for all of the meshes in our scene
		/// </summary>
		List<CasterCollider> _allMeshes = new List<CasterCollider>();
		...
		/// </summary>
		Vector3 _lastPos = Vector3.forward;
		...
		/// </summary>
		float _magRange = 0.01f; //25.15f; //last value 0.15f;
		...
		/// </summary>
		bool _EnumeratorOnEnterOnExitIsRunning = false;
		...
		public void Rebuild() {
			...
			if (renderer == null) {
				renderer = gameObject.AddComponent(typeof (MeshRenderer)) as MeshRenderer;
					// Add a Mesh Renderer component to the light game object so the form can become visible
			}
		...
		public void GetCollidersOnScene() {
			...
			for (int j = 0; j < _allBoxes.Length; j++) {
				if (_allBoxes[j].enabled)
					_allMeshes.Add(new CasterCollider(_allBoxes[j]));
			}
			...
			for (int j = _allBoxes.Length; j < (_allBoxes.Length + _allPolygons.Length); j++) {
				if (_allPolygons[j - _allBoxes.Length].enabled)
					_allMeshes.Add(new CasterCollider(_allPolygons[j - _allBoxes.Length]));
			}



			// -- Circles --//
			for (int j = _allBoxes.Length + _allPolygons.Length;
				j < (_allBoxes.Length + _allPolygons.Length + _allCircles.Length);
				j++) {
				if (_allCircles[j - (_allBoxes.Length + _allPolygons.Length)].enabled)
					_allMeshes.Add(new CasterCollider(_allCircles[j - (_allBoxes.Length + _allPolygons.Length)],
						(Vector2) transform.position));
			}

			// -- Edges --//
			for (int j = _allBoxes.Length + _allPolygons.Length + _allCircles.Length;
				j < (_allBoxes.Length + _allPolygons.Length + _allCircles.Length + _allEdges.Length);
				j++) {
				if (_allEdges[j - (_allBoxes.Length + _allPolygons.Length + _allCircles.Length)].enabled)
					_allMeshes.Add(new CasterCollider(_allEdges[j - (_allBoxes.Length + _allPolygons.Length + _allCircles.Length)]));
			}


			//Resize array
			_allMeshes.RemoveAll(c => c == null);
			...
			_colliders = new Collider[_allMeshes.Count];

			// Construct the colliders
			for (int i = 0; i < _allMeshes.Count; i++)
		...
		public void GetCollidersOnSceneNonStatic() {
			...
			for (int i = 0; i < _allMeshes.Count; i++) {
				if (cols[i] is BoxCollider2D)
					_allMeshes.Add(new CasterCollider((BoxCollider2D) cols[i]));
				else if (cols[i] is PolygonCollider2D)
					_allMeshes.Add(new CasterCollider((PolygonCollider2D) cols[i]));
				else if (cols[i] is CircleCollider2D)
					_allMeshes.Add(new CasterCollider((CircleCollider2D) cols[i], (Vector2) transform.position));
				else if (cols[i] is EdgeCollider2D)
					_allMeshes.Add(new CasterCollider((EdgeCollider2D) cols[i]));
			}
			...
			// let's preAllocate thte max ammount so we don't have to wait for expansions later on
		...
		void LateUpdate() {
			...
			if ((anyColliderChange != ColliderState.NoChanged) || (transform.position != _lastPos) ||
				(transform.rotation != _lastRot) || (_forceToRefresh) || (LightRadius != _lastRadius)) {
		...
		ColliderState DidAnyColliderTMChange() {
		...
		void GenerateColliderVerts() {
			...
			for (int m = 0; m < _colliders.Length; m++) {
				...
				var obj = _colliders[m].GetVerts(transform, LightRadius, RangeAngle, ref _sortAngles, Layer, _magRange,
					ref notifyGameObjectsReached, ref objReached, ref debugLines, forceToUpdatePoints);
				...
				worldCount += _colliders[m].worldPoints.Length;
						...
						if (isEndpoint == true) {
							...
							if (_ray) {
								...
								if (debugLines == true)
									...
									Debug.DrawLine(new Vector3(from.x, from.y, transform.position.z),
										new Vector3(hitp.x, hitp.y, transform.position.z), Color.blue);
							...
							if (debugLines == true && _ray)
								Debug.DrawLine(new Vector3(fromCast.x, fromCast.y, transform.position.z),
									new Vector3(hitp.x, hitp.y, transform.position.z), Color.green);
		...
		internal void DeleteSuperflousVerts(ref VertItem item) {
			...
			for (int i = 0; i < _allVerts.Count(); i++) {
				...
				for (int j = 0; j < item.Verts.Count(); j++) {
					...
					if (_allVerts[i].angle >= (item.Verts[j].angle - _rangeComparative) &&
						_allVerts[i].angle <= (item.Verts[j].angle + _rangeComparative)) {
...
#if UNITY_EDITOR
		void checkOnEnterOnExitEventsEditor() {
		...
		void GenerateLightVectors() {
			...
			for (int i = 0; i <= Segments; i++) {
				...
				Vert v = new Vert();
				v.pos = new Vector3((Mathf.Sin(theta * Mathf.Deg2Rad)), (Mathf.Cos(theta * Mathf.Deg2Rad)), 0);
					// in radians, calculate on the fly, low performance
		...
		static void SwapVertOrders(ref List<Vert> allVerts) {
		...
		Vector3[] _initVerticesMeshLight; // these will also land on the heap, not the stack
		Vector2[] _uvs;
		Vector3[] _normals;
		Color32[] _colors;
		int[] _triangles;
		...
		void RenderLightMesh() {
			...
			for (int i = 0; i < _initVerticesMeshLight.Length; i++) {
				_uvs[i] =
					(new Vector2((uv_Offset.x + (_initVerticesMeshLight[i].x * uv_Scale.x) / LightRadius),
						(uv_Offset.y + (_initVerticesMeshLight[i].y * uv_Scale.y) / LightRadius)));
