Shader "2D Dynamic Lights/RadialGradient/AlphaBlend" {
	...
	Properties {
		[hdr] _Color ("Color", Color) = (1,1,1,1)				//Receive input from a fixed Color
	...
	SubShader {
		...
		Pass {
			...
			float4 frag(VertexToFragment i) : COLOR {
				...
				float faded = sqrt((i.uv.x*i.uv.x) + (i.uv.y*i.uv.y)) + _Offset;

				return float4(lerp(_Color,fixed4(_Color.rgb,0), saturate(faded)));	//Output radial gradient
			}
